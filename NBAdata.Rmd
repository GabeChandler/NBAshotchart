...
```{r}
TeamInfo <- read.table("~/TeamInfo", quote="\"", comment.char="", header = TRUE)
...
Import the schedule
```{r}
sched <- read.csv("~/nba-full-schedule-2019-2020.csv")
```

Let's find all the Suns' games
```{r}
suns.played <- which(sched$away_team=='PHX' | sched$home_team=='PHX')[1:65]
suns.games <- list()
for (i in 1:length(suns.played))
  suns.games[[i]] <- nba.data(suns.played[[i]])
```
